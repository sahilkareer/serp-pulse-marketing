...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    description: d?.seoDesc || 'Simple, transparent pricing. AI tracking, white-label reports, MCP server — all included. 30-day free trial on every tier.',
  }
}
...
const DEFAULT_PLANS = [
  { tier: 'Freelancer', monthly: 20, annual: 19, desc: 'For solo SEOs and personal projects.', cta: 'Start 30-Day Free Trial', ctaStyle: 'outline' as const, features: ['1 GSC project','1 GA4 property','1 white-label report','GSC + GA4 analytics','AI citation tracking (16+ platforms)','MCP Server access','CSV exports','Light/dark theme'], missing: ['Global filters','White-label branding'] },
  { tier: 'Pro', monthly: 49, annual: 46, desc: 'For SEO professionals managing multiple clients.', cta: 'Start 30-Day Free Trial →', ctaStyle: 'filled' as const, popular: true, features: ['10 GSC projects','10 GA4 properties','10 white-label reports','Everything in Freelancer','Global filters across all data','All 9 GA4 dashboard widgets','Advanced date comparisons','Alerts & notifications','Priority support'], missing: [] },
  { tier: 'Agency', monthly: 159, annual: 150, desc: 'Unlimited scale for agencies and teams.', cta: 'Start 30-Day Free Trial →', ctaStyle: 'outline' as const, features: ['Unlimited GSC projects','Unlimited GA4 properties','Unlimited reports','Everything in Pro','White-label branding','Team access & collaboration','API access','Beta features early access','Dedicated support'], missing: [] },
]
...
export default async function PricingPage() {
  ...
  const subtext   = d?.heroSubtext   || 'AI tracking, white-label reports, MCP server — all included from your first paid plan. 30-day free trial on every tier.'
  ...
  const ctaSubtext = d?.ctaSubtext  || '30-day free trial on every plan. No credit card. Connect in 2 minutes.'
    ...
    <>
      ...
      <section style={{padding:'0 24px 80px',background:'var(--bg)'}}>
        <div style={{textAlign:'center'}}>
          <PricingCards plans={DEFAULT_PLANS} comparison={DEFAULT_COMPARISON} annualDiscount={annualDiscount} />
        </div>
