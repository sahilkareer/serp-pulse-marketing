  ...
  const plans = [
    {
      ...
      features: [
        '1 GSC project',
        '1 GA4 property',
        '1 white-label report',
        'GSC + GA4 analytics',
        'AI citation tracking (16+ platforms)',
        'MCP Server access',
        'CSV exports',
        'Light/dark theme',
      ],
    ...
    },
    {
      ...
      features: [
        ...
        'Global filters across all data',
        'All 9 GA4 dashboard widgets',
  ...
  return (
    <>
      ...
      <section style={{padding:'120px 24px 60px',textAlign:'center',background:'var(--bg)',position:'relative',overflow:'hidden'}}>
        <div style={{position:'absolute',top:-200,left:'50%',transform:'translateX(-50%)',width:800,height:500,background:'radial-gradient(ellipse,rgba(8,145,178,.04),transparent 65%)',pointerEvents:'none'}}/>
        <div style={{position:'relative',zIndex:1,maxWidth:640,margin:'0 auto'}}>
          ...
          <p style={{fontSize:12,color:'var(--gn)',fontWeight:600,background:'var(--gns)',padding:'5px 16px',borderRadius:8,display:'inline-block',marginBottom:0}}>30-day free trial on all plans · No credit card required</p>
        </div>
      </section>
        ...
        <div style={{maxWidth:1000,margin:'0 auto',display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:16,alignItems:'start'}} className="how-grid">
          {plans.map(plan=>(
            <div key={plan.tier} className="rv" style={{
              background:'var(--wh)',
              border:plan.popular?'2px solid var(--tl)':'1.5px solid var(--bd)',
              borderRadius:'var(--rl)',
              padding:'28px 24px',
              position:'relative',
              ...
              <div style={{fontSize:11,color:'var(--mt2)',marginBottom:6}}>
                {annual ? `Billed annually · $${plan.monthly}/mo monthly` : 'Billed monthly'}
              </div>
              ...
              <ul style={{listStyle:'none',padding:0,marginBottom:22,display:'flex',flexDirection:'column',gap:8}}>
                {plan.features.map(f=>(
                  <li key={f} style={{display:'flex',alignItems:'flex-start',gap:8,fontSize:13,color:'var(--ink3)'}}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#10b981" strokeWidth="2.5" style={{flexShrink:0,marginTop:1}}><path d="M20 6L9 17l-5-5"/></svg>
                    {f}
                  </li>
                ))}
                {plan.missing.map(f=>(
                  <li key={f} style={{display:'flex',alignItems:'flex-start',gap:8,fontSize:13,color:'var(--mt3)',textDecoration:'line-through'}}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--mt3)" strokeWidth="2" style={{flexShrink:0,marginTop:1}}><path d="M18 6L6 18M6 6l12 12"/></svg>
                    {f}
                  </li>
                ))}
              </ul>
              ...
              <a href={`${APP}/signup`} style={{
                display:'block',textAlign:'center',padding:'12px',borderRadius:9,
                fontSize:13,fontWeight:600,textDecoration:'none',transition:'all .15s',
                fontFamily:'var(--hd)',
                ...(plan.ctaStyle==='filled'
                  ? {background:'var(--ink)',color:'white',border:'1.5px solid var(--ink)'}
                  : {border:'1.5px solid var(--bd2)',color:'var(--mt)'})
              }}>{plan.cta}</a>
            </div>
          ))}
        </div>

        <p style={{textAlign:'center',fontSize:13,color:'var(--mt2)',marginTop:24}}>
          All plans include a <strong style={{color:'var(--ink)'}}>30-day free trial</strong>. No credit card required. Cancel anytime.
        </p>
      </section>
      ...
      <section style={{padding:'80px 24px',background:'var(--wh)'}}>
        <div style={{maxWidth:900,margin:'0 auto'}}>
          ...
          <div style={{overflowX:'auto'}}>
            <table style={{width:'100%',borderCollapse:'collapse',border:'1px solid var(--bd)',borderRadius:'var(--rl)',overflow:'hidden'}}>
              <thead>
                <tr>
              ...
              <tbody>
                ...
                <tr>
                  <td style={{padding:'16px 20px',fontWeight:700,fontSize:13,color:'var(--ink)'}}></td>
                  {[{cta:'Start Free',href:`${APP}/signup`,style:{border:'1.5px solid var(--bd2)',color:'var(--mt)',background:'transparent'}},{cta:'Start Free →',href:`${APP}/signup`,style:{background:'var(--ink)',color:'white',border:'1.5px solid var(--ink)'}},{cta:'Start Free →',href:`${APP}/signup`,style:{border:'1.5px solid var(--bd2)',color:'var(--mt)',background:'transparent'}}].map((btn,i)=>(
                    <td key={i} style={{padding:'16px 12px',textAlign:'center'}}>
                      <a href={btn.href} style={{...btn.style,display:'inline-block',padding:'9px 18px',borderRadius:8,fontSize:12,fontWeight:600,textDecoration:'none',fontFamily:'var(--hd)'}}>{btn.cta}</a>
                    </td>
                  ))}
                </tr>
              </tbody>
      ...
      <section className="final dark-sec">
        ...
        <div className="w">
          ...
          <div className="final-checks">
            <span className="fck">30-day trial</span>
